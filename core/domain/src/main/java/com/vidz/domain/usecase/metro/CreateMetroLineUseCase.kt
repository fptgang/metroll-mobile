...
class CreateMetroLineUseCase @Inject constructor(
    private val repository: MetroLineRepository
) {
    suspend operator fun invoke(metroLine: MetroLine): Flow<Result<MetroLine>> {
        return repository.createMetroLine(metroLine)
    }
} 